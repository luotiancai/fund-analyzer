...
#   8    6/8=75.0%   +490.48%           76天      +30.38%
#   (剔除单笔运气021528财通成长优选混合C+140.23%后, 剩7笔仍有+145.80%,
#   两笔小亏-0.06%/-0.37%——8笔里只有这2笔亏,都亏得极小)
#   加规模门槛前(min_aum=None)是 +470.41%: 门槛把2026-03那笔几十万规模的
#   东方城镇消费主题C(+3.10%)换成了正常规模的广发医药创新发起式C(-0.37%,
#   跌幅更深、被止损兜住), 另外2022-10/2023-08两笔标的也一并升级。5000万
#   vs 1亿量级差别不大, 选5000万即可。
#   注: 2026-07-29 修正规模披露日口径(季报四季度都是季末后~1个月披露, 之前
#   误按半年报/年报的62/92天), 使2026-03那笔从国融融盛(+30.83%)校正为广发
#   医药创新(-0.37%,跌得更深且当天规模已够), 累计从+675.40%回落到+490.48%
#   ——这是修 bug 后的真实值, 之前偏高是用了尚不该可见的更旧季度规模所致。
#
...
# 2024-09-26 华富健康文娱灵活配置混合C(019200)  混合型-灵活  1.93 20.45  7.89 4.09  -22.41%  2024-10-09 +18.05%(费后+17.55%) 0.5 +32.8% 11.1% +8.6% 基金11.1%>=7.9%+大盘6.6%>=4.1%(同日双触发)
...
# 2026-03-23 广发医药创新混合发起式C(017963)   混合型-偏股  2.68 22.54 12.08 4.51  -19.69%  2026-06-05 -0.37%    0.0 +14.2% 12.8% +5.6%  基金12.8%>=12.1%+大盘5.1%>=4.5%(同日双触发)
