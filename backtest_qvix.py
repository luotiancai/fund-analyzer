...
# ── 标准策略回测参考快照(2026-07-28, 数据/规则变了要重跑, 别当成结论)──
...
#   8    7/8=87.5%   +675.40%           75天      +34.29%
#   (剔除单笔运气021528财通成长优选混合C+140.23%后, 剩7笔仍有+222.77%,
#   最大亏损-0.06%,平均亏损-0.06%——8笔里只有这1笔亏,而且亏得极小)
#   加规模门槛前(min_aum=None)是 +470.41%: 门槛把2026-03那笔几十万规模的
#   东方城镇消费主题C(+3.10%)换成了正常规模的国融融盛龙头严选C(+30.83%),
#   另外2022-10/2023-08两笔标的也一并升级。5000万 vs 1亿实测几乎打平
#   (1亿是+610.37%,唯一差别是2024-02那笔的替换),量级选5000万即可。
#
...
# 2022-10-24 富国中证港股通互联网ETF发起式联接C(014674) 指数型-股票 2.19 23.24 10.18 4.65 -26.93% 2022-12-22 +49.70% 0.0 +54.7% 8.6% +2.6% 大盘4.9%>=4.6%
# 2023-08-28 诺安积极回报混合C(012847)         混合型-灵活  3.20 22.28 14.26 4.46  -27.95%  2023-10-19 -0.06%    0.0  +8.5% 12.5% -3.0%  大盘5.4%>=4.5%
...
# 2026-03-23 国融融盛龙头严选混合C(006719)     混合型-偏股  2.49 22.54 11.22 4.51  -18.57%  2026-06-01 +30.83%   0.0 +51.3% 13.5% +6.4%  基金13.5%>=11.2%
...
def run_backtest(window: int = 490, pct: float = 0.90, minp_ratio: float = 0.97,
    ...
    主题C, 规模仅几万)。实测加0.5亿门槛后累计收益(费后复利)从+470.41%升到
    +675.40%, 胜率仍7/8, 主要是把那笔垃圾换成正常规模标的; 5000万 vs 1亿
    (+610.37%)几乎打平, 量级选5000万即可。传 None 关掉过滤。"""
    conn = get_conn()
