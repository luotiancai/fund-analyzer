...
# ── 标准策略回测参考快照(2026-07-24, 数据/规则变了要重跑, 别当成结论)──
...
# 求其次买涨幅最小的) + 排除QDII/海外/港股通/持有期锁定基金, 命令:
#   python3 backtest_qvix.py --window 490 --pct 0.90
# (以上都是当前 run_backtest 的默认值, 不用额外传参)
#
#   笔数  胜率        累计收益(费后复利)  平均持有  平均收益(费后)
#   9    7/9=77.8%   +317.69%           63天      +22.35%
#   (剔除单笔运气021528财通成长优选混合C+140.23%后, 剩8笔仍有+73.87%,
#   最大亏损-9.76%,平均亏损-5.26%)
#
...
# 2022-10-24 华安优势精选混合C(014540)     混合型-偏股  1.55 23.24  7.20 4.65  -24.17%  2022-11-23 +6.74%               0.0 +15.0%  7.2% +4.0%  基金7.2%>=7.2%
# 2022-11-29 富安达新兴成长混合C(014471)   混合型-灵活  2.13 23.29  9.92 4.66  -24.58%  2022-12-22 -9.26%(费后-9.76%)   0.5  +2.2% 11.2% -3.0%  基金11.2%>=9.9%
# 2023-08-28 诺安稳健回报混合C(002052)     混合型-灵活  3.07 22.28 13.68 4.46  -31.63%  2023-10-19 -0.77%               0.0  +7.2% 10.7% -3.0%  大盘5.4%>=4.5%
# 2024-02-05 东吴新经济混合C(012617)       混合型-偏股  1.89 22.28  8.42 4.46  -36.76%  2024-03-27 +23.24%              0.0 +36.3%  9.6% +10.8% 基金9.6%>=8.4%
# 2024-09-26 华富健康文娱灵活配置混合C(019200) 混合型-灵活 1.93 20.45 7.89 4.09 -22.41% 2024-10-09 +18.05%(费后+17.55%)  0.5 +32.8% 11.1% +8.6%  基金11.1%>=7.9%
# 2024-10-10 海富通科技创新混合C(009024)   混合型-偏股  1.81 20.52  7.43 4.10   -2.42%  2024-11-14 +6.66%               0.0 +15.9%  8.0% +2.4%  基金8.0%>=7.4%
# 2025-04-07 财通成长优选混合C(021528)     混合型-灵活  3.41 21.42 14.61 4.28  -22.35%  2025-11-18 +140.23%             0.0 +181.4% 14.6% +27.2% 基金14.6%>=14.6%
# 2026-03-23 东方城镇消费主题混合C(025884) 混合型-偏股  2.34 22.54 10.55 4.51  -20.75%  2026-05-21 +3.10%               0.0 +17.1% 11.9% +6.9%  基金11.9%>=10.5%
...
# 港股/沪港深/恒生指数系列基金也跟踪境外(香港)市场, 但走的是"港股通"
# 通道而不是QDII额度, type 字段常常只是普普通通的"指数型-股票"/"混合
# 型-偏股", 不含"QDII"/"海外"字样, 会被 qdii_codes 那道过滤漏掉(2022
# 年10-12月那波港股互联网超跌反弹实测漏过一只"国泰中证港股通科技ETF
# 发起联接C", 已核实剔除)。跟QDII基金是同一个排除理由: 跟QVIX/大盘
# 恐慌-反弹逻辑脱钩。
...
def run_backtest(window: int = 720, pct: float = 0.95, minp_ratio: float = 0.97,
    ...
    里只挑出9笔真正"深跌+高波动"的交易, 胜率77.8%(7/9), 累计收益(费后
    复利)+317.69%, 剔除单笔运气(021528财通成长优选混合C+140.23%)后
    仍有+73.87%, 明显优于不设"跌幅耗尽即放弃"时的版本。另外这批候选
    统一排除 QDII/海外/港股通(见 exclude_codes 里的 qdii_codes/
    hk_codes)——港股通渠道的基金 type 字段不含"QDII"/"海外"字样,
    实测漏过一只"国泰中证港股通科技ETF发起联接C"(2022年10-12月港股
    互联网超跌反弹段选中, +31.54%), 排除它会让这版回测收益变差(是
    该排除规则的应有代价, 不是bug)。
    ...
    # 港股通/沪港深/恒生系列也排除(见 _HK_RE 定义处说明,QDII过滤漏网的
    # 同类境外敞口)
    hk_codes = {c for c, n in fund_names.items() if _HK_RE.search(n)}
    # 有持有期锁定的基金也排除(见 _HOLD_PERIOD_RE 定义处说明)
    hold_codes = {c for c, n in fund_names.items() if _HOLD_PERIOD_RE.search(n)}
    exclude_codes = qdii_codes | hk_codes | hold_codes
