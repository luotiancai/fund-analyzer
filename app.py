...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        _qvix_hist = None
        if _show_vix:
            _q = load_qvix_self(fetcher.qvix_self_history_last_date())
            ...
            if _qvix_hist is None or _qvix_hist.dropna(subset=["qvix"]).empty:
                st.caption("⚠️ VIX恐慌指数数据暂不可用")
                _qvix_hist = None
        if _qvix_hist is not None:
            _line = _qvix_hist.dropna(subset=["qvix"])
            fig_sse.data[0].name = "上证指数"
            fig_sse.data[0].showlegend = True
            fig_sse.add_trace(go.Scatter(
                x=_line["date"], y=_line["qvix"],
                name="VIX恐慌指数(QVIX,自算)", yaxis="y2",
                line=dict(color=_QVIX_COLOR, width=1.3),
            ...
            if _t is not None and not _t.empty:
                fig_sse.add_trace(go.Scatter(
                    ...
                    line=dict(color=_QVIX_COLOR, width=1.2, dash="dash"),
                    hovertemplate="阈值 %{y:.2f}<extra></extra>"))
                # 链失效日在曲线上只是个断点, 什么都看不见 —— 而它恰恰是最
                # 恐慌的那几天(标的跌出期权链的挂牌行权价范围, 公式失效;
                # 见 qvix_calc.CHAIN_BROKEN)。在阈值线的高度上打个 ×, 位置
                # 不代表数值(那天没有数值), 只代表"这天在阈值之上"。
                _bk = _qvix_hist[
                    _qvix_hist["note"].map(qvix_calc.is_chain_broken)
                ][["date"]].merge(_t, on="date", how="inner")
                if not _bk.empty:
                    fig_sse.add_trace(go.Scatter(
                        x=_bk["date"], y=_bk["thr"],
                        name="链失效(按信号处理)", yaxis="y2",
                        mode="markers",
                        marker=dict(symbol="x", size=12, color=_QVIX_COLOR,
                                    line=dict(width=2)),
                        hovertemplate="那天算不出来——跌出期权链行权价范围,"
                                      "公式失效,按信号处理<extra></extra>"))
        ...
        with _tab_review:
            st.markdown(
                ...
                "。卖出后隔 2 个交易日才允许下一次买入(赎回款 T+2 到账,"
                "卖出当天和次日买不进去)")
            ...
            if _bt_df is None or _bt_df.empty:
                ...
                st.caption(
                    ...
                    "全部落在7亿以下——但那个观察是在2~10亿的池子里得出的、属于循环论证,"
                    "2026-08-13 已撤掉上限:去掉后 8 笔里有 3 笔换成 19~24 亿的大基金,"
                    "两胜一负净赚 40 个点(复利 +160 个点);30%跌幅上限的依据——事后最优解的近3月跌幅"
                    "没有一只超过-27%,而跌得最深的那笔往往是\"跌的市场和反弹的市场"
                    "不是同一个\");"
                    "当天找不到真跌的合格候选就放弃当天、顺延到下一个信号日"
                    "——2026-08-12 之前这里是\"改买涨幅最大的那只\","
                    "已删除:那是动量逻辑,跟\"恐慌到位→超跌反弹\"的入场前提"
                    "自相矛盾,而且跌幅耗尽本身就说明前提不成立。三笔兜底交易"
                    "是 +3.55%/-3.20%/+36.76%,除北证50那笔外接近于零,而那笔"
                    "同一只基金在止损后连续12个信号日仍是涨幅冠军、照规则买回去"
                    "是 -10.47%,说明 +36.76% 是买入时点的运气而非规则的能力。"
                    "规模按信号日当时"
                ...
                _review_df = _review_df[[c for c in [
                    ...
                    # 「可买池分位」= 这一笔在**当天本来可以买的所有基金**里排第几
                    # (打败了百分之多少)。分母跟「可达天花板」那条对照跑批一致:
                    # 规模≥2亿 + 排除 QDII/海外、持有期锁定、净值脏数据, 其余不设
                    # 条件; 每只候选都按同一套双止损跑出实现收益, 跟本笔可比;
                    # 50% 就等于闭眼瞎选。
                    # 它**不是**策略自己的候选池(策略还要过"真跌+跌幅≤30%+波动
                    # ≥1.5"三道筛选) —— 这是故意的: 分母若用策略筛完的名单, 衡量
                    # 的就只剩"在选好的候选里排第几", 等于给那几道筛选免票, 而它们
                    # 恰恰是策略的一部分、也最该被检验。
                    # 分母为什么要卡 2亿: 不卡的话混进的是策略根本不会买的迷你
                    # 基金, 而且那批净值本身不可信(015111 在 2024-03-28 单日
                    # +20.18%, 当季末净资产只剩100万 —— 那是93%份额被短期赎回、
                    # 赎回费分摊给剩下7%造成的, 不是投资收益)。
        ...
        with _tab_runs:
            if not _runs:
                ...
                if _x_df is None or _x_df.empty:
                    ...
                    # 有些跑批的选法根本不是 pick 那几种(如"事后最佳上限":
                    # 每笔取当天候选池里自己跑完双止损后实现收益最高的那只),
                    # 拿 pick 去套会把规则描述得完全不对。这类跑批自己在
                    # params 里带 rule_note/exit_note, 有就顶掉自动拼的那句。
                    _rule = _x_params.get("rule_note") or _rule
                    ...
                    # 涨幅分支门槛: 老跑批没记这项, 那时两个分支同用一个数
                    _tvr = _x_params.get("top_min_vol_ratio", "same")
                    _mdrop = _x_params.get("max_drop")   # 老跑批没记这项
                    ...
                    st.caption(
                        ...
                        + ("(仅跌幅分支, 涨幅分支不设门槛)" if _tvr is None
                           else "(涨跌两个分支同一门槛)" if _tvr == "same"
                           else f"(跌幅分支; 涨幅分支≥**{_tvr}**)")
                        + "、"
                        + (f"近3月跌幅≤{_mdrop}%、" if _mdrop == 30.0
                           else f"近3月跌幅≤**{_mdrop}%**、" if _mdrop
                           else "**跌幅不设上限**、")
                        ...
                        + (f",{_x_params['exit_note']}"
                           if _x_params.get("exit_note") else ",双止损口径同标准策略")
                        + f"(粗体=与标准策略不同的参数)。"
        ...
        with _tab_daily:
            ...
            if _qvix_hist is not None:
                _q = _qvix_hist.assign(
                    ...
                    **{"VIX恐慌指数": pd.to_numeric(_qvix_hist["qvix"],
                                                errors="coerce").round(2)})
