        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            st.dataframe(pd.DataFrame({
                "买入日": ["2019-02-25", "2019-05-06",
                          ...
                          "2024-10-09", "2025-04-07", "2025-08-25",
                          "2026-03-23"],
                "冠军(C类全市场,按前一交易日榜单)": [
                    "前海开源价值成长混合C (006217)",
                ...
                "类型": ["混合型-灵活", "混合型-灵活", "混合型-偏股", "股票型",
                        ...
                        "混合型-偏股(机器人)", "混合型-偏股(CPO算力)",
                        "混合型-偏股"],
                "冠军近3月涨幅(前日口径)": [
                    "+27.48%", "+49.28%", "+45.41%",
                    "+38.48%", "+25.72%", "+56.07%", "+21.37%", "+13.61%",
                    "+25.77%", "+61.40%", "+63.60%", "+115.76%", "+47.23%"],
                "卖出日": ["2019-04-26", "2019-07-22",
                          ...
                          "2025-12-16", "2026-06-05"],
                "持有收益": ["+10.69%", "+0.95%",
                            "+22.11% (费后+21.50%)",
                            "-12.05% (费后-13.37%)",
                            "+32.52%", "-3.17%", "+10.34%", "+15.92%",
                            "+3.04% (费后+2.52%)",
                            "+33.24%", "+27.74%", "+30.14%", "+39.08%"],
                "期间最高": ["+18.1%", "+6.8%", "+34.2%",
                            "+0.0%", "+51.6%", "+10.5%", "+27.6%", "+21.7%",
                            "+9.5%", "+43.9%", "+62.5%", "+38.2%", "+50.6%"],
                "期间最大回撤": ["6.3%", "6.7%", "9.0%",
                              "12.1%", "12.6%", "12.4%", "13.5%", "10.2%",
                              "5.9%", "12.2%", "21.4%", "18.4%", "12.6%"],
                "同期上证": ["+4.2%", "-0.7%", "+4.8%",
                ...
                "点评": [
                    ...
                    "(15% 线会在 5-6 月洗盘中被抖出局:6/19 触发仅 +7.6%,"
                    "还跑输同期上证的 +8.6%)",
                    "亢奋日触发(+1.5% 上涨日,QVIX 25.78 破阈值 24.88)"
                    ";冠军 CPO 动量与大盘脱钩,10/29 冲"
                    " +38.2%;上证 11/13 见顶 4029 后阴跌,12/16 大盘 5% 线"
                    "离场 +30.1%,跑赢同期上证 31 个点;基金自身最深回撤"
                    " 18.4% 擦过 20% 线未触发(15% 线会在 11/21 被抖出局,"
                    "仅 +14.2%)",
            ...
            st.caption(
                "口径:C 类全市场(含债基/固收+),仅剔除 QDII/海外/黄金等与"
                " A 股脱钩的资产,以及巨额赎回假净值(迷你基金月涨 5%~7% 的"
                "赎回费入账)。冠军按买入日前一交易日的近3月榜单选出(当日净值"
                "盘中未公布,这是实盘真实可见的口径,无前视偏差);买入按当日"
                "净值成交,持有收益不含买入日当天涨跌。费后收益按 C 类赎回费"
                "扣减:持有<7 天 1.5%、7~30 天 0.5%、≥30 天免(乘法口径,"
                "(1+r)×(1-费率)-1),持有天数按份额确认日起算;受影响的仅"
                "三笔——2020-02-03(27天,0.5%)、2020-03-12(6天,1.5%)、"
                "2024-09-27(确认 9/30、赎回确认 10/10,持有 10 天,0.5%)。"
                "三步规则:恐慌急跌日触发"
                "(QVIX 突破滚动 3 年 95 分位阈值——不论当日涨跌,或 -5% 级暴跌;"
                ...
                "抖出局;案例中仅 2025-04-07 由基金线先触发)。2018-02-09/"
                "2018-10-29 两例已按动态阈值口径剔除:当日 QVIX 33.1/31.8,"
                "低于各自的 3 年 95 分位阈值 47.2/33.5(2015 股灾读数仍在窗口"
                "内),不构成触发日——那两笔恰好也是亏的,阈值动态化把它们"
                "天然过滤掉了。")
...
with tab_gold:
    ...
    if au_df is None or au_df.empty:
        ...
        # 距前高回撤:全史滚动峰值口径,对应"回撤≥15%离场"规则的当前状态。
        _aformerpeak = float(au_all["close"].cummax().iloc[-1])
        _add_now = (1.0 - _alatest["close"] / _aformerpeak) * 100.0
        ...
        ak4.metric("距前高回撤", f"-{_add_now:.1f}%",
                   "≥15% 离场区" if _add_now >= 15.0 else "<15% 持有区",
                   delta_color="inverse" if _add_now >= 15.0 else "normal")
        ak5.metric("数据日期", _alatest["date"].strftime("%Y-%m-%d"))
        ...
        # 收盘价离线重算后硬编码。新案例由用户口述买入日后补录。
        with st.expander("📜 黄金策略复盘:高点回撤15%卖出"):
            st.dataframe(pd.DataFrame({
                "买入日": ["2019-06-24", "2022-03-07", "2026-02-02 (重置)",
                          "2026-03-23 (重置)"],
                "买入价(元/克)": ["313.26", "401.50", "1030.00", "924.65"],
                "卖出日": ["2020-11-25", "2026-02-02", "2026-03-23",
                          "2026-06-11"],
                "卖出价(元/克)": ["378.45", "1030.00", "924.65", "896.01"],
                "持有收益": ["+20.8%", "+156.5%", "-10.2%", "-3.1%"],
                "持有天数": ["520", "1428", "49", "80"],
                "期间最高": ["+42.4% (2020-08-07)", "+209.6% (2026-01-29)",
                            "+16.5% (2026-03-02)", "+14.6% (2026-04-08)"],
                "期间最大回撤": ["15.1% (触发线)", "17.1% (触发线)",
                              "22.9% (单日-11.2%破线)", "15.5% (触发线)"],
                "点评": [
                    "美联储转鸽、金价突破 1400 美元的起点买入;2020-08 冲顶"
                    " +42.4% 后进入顶部震荡,11-25 触发 15% 线离场。5% 线会在"
                    " 2019-10 就被洗出(仅 +8.5%),10% 线会在 2020-03 新冠"
                    "挤兑中被抖出(仅 +5.7%)——15% 线是全组最佳",
                    "俄乌开战冲高日买入(当日 +1.9%);2022 美元金价深调 -22%,"
                    "人民币口径靠汇率缓冲仅回撤 ~9%,完整拿住 2023-2025 超级"
                    "牛市;2026-01-29 见顶 +209.6% 后两日暴跌 17% 触线,当日"
                    " GVZ 37.4 远超阈值 24.5(恐慌急跌不卖),按例外规则结转"
                    "本段、重置高点续监听下一段",
                    "二次监听段:V 弹 +16.5% 至 1199 未创前高,03-23 单日"
                    " -11.2% 破线;当日 GVZ 43.4 仍超阈值 30.7(恰逢 A 股"
                    " QVIX 冲 42 的全球恐慌日),继续不卖,再次结转重置",
                    "三次监听段:反弹 +14.6% 至 1060(4-08)后阴跌,06-11"
                    " 距峰 -15.5% 触线,当日 GVZ 28.3 已回落至阈值 33.1 下方"
                    "——非恐慌下跌,真离场。三段合计 +123.2%(vs 02-02 直接"
                    "卖 +156.5%:例外规则本轮少赚 33 个点,买的是创新高就"
                    "继续持有的期权);离场后金价续跌至 869(7-01),方向仍对"],
            }), use_container_width=True, hide_index=True)
            st.caption(
                "口径:Au99.99 收盘价成交,自买入后最高收盘回撤≥15% 当日"
                "收盘卖出;例外:触发日 GVZ 若在滚动 3 年 95 分位阈值上方"
                "(恐慌性急跌,存在 V 弹可能),不卖,把当段结转为独立一行,"
                "从当日重置高点重新监听 15%,直至某次在阈值下方触发才真正"
                "离场(2022 案例走了三段:02-02、03-23 两次超阈值结转,"
                "06-11 阈值下方真离场)。无恐慌买点信号,入场由人工判断,"
                "GVZ 只作波动参考。定线依据(2026-07 回测):2017-2026 十轮"
                "回撤天然分两档——正常洗盘 5%~12%,真顶部 20%+;15% 取"
                "空档稳健位,洗盘够不到、真顶部必触发。")
