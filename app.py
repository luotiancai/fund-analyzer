    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            st.dataframe(pd.DataFrame({
                ...
                "同期上证": ["+4.2%"],
                "点评": [
                    "前日口径避开了涨停当天才冲上榜首的券商,买到均衡成长冠军,"
                    "4 月冲顶 +18.1% 后随大盘 5% 线离场落袋 +10.7%;持有 60 天"
                    "免赎回费,基金 20% 线全程未触发",
                ],
            ...
            st.caption(
                ...
                "(1+r)×(1-费率)-1),持有天数按份额确认日起算。三步规则:"
                "恐慌急跌日触发(QVIX 突破滚动 3 年 95 分位阈值且当日下跌,"
                "或 -5% 级暴跌;固定 30 有时代漂移问题,见图上动态阈值线)→ "
                "看榜单结构定攻防(权益霸榜买权益冠军,债基霸榜买固收冠军)→ "
                "离场取先触发者:大盘从买入后高点回撤 5%,或基金自身从买入后"
                "高点回撤 20%(20% 高于高贝塔冠军 12~15% 的日常洗盘振幅,"
                "10%/15% 线会被正常震荡抖出局)。")
