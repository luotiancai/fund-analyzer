...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # 脏数据)。数据 2018 年起,2018 全年无信号日(滚动3年窗口含 2015
        # 股灾,阈值 32~47 高于全年 QVIX 峰值 33.06)。
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("17 笔已完成:胜率 64.7%(11/17),费后复利 +264.84%,"
                       "累计手续费 6.0%,平均持有 48 天,平均费后 +8.79%,"
                       "最佳 +39.10% / 最差 -11.38%。"
            ...
            _review_df = pd.DataFrame({
                ...
                "波动率比值(近3月)": [1.33, 1.76, 2.71, 1.61, 1.63, 1.48, 1.18,
                                   1.84, 1.00, 1.47, 1.65, 1.27, 1.69, 1.83,
                                   2.68, 5.62, 3.82],
                ...
                "回撤控制线(%)": [7.53, 10.34, 15.94, 9.47, 9.66, 9.16, 7.33,
                                10.93, 4.69, 6.80, 7.68, 5.94, 8.04, 8.98,
                                13.41, 27.97, 19.01],
                ...
                "备注": [
                    ...
                    "(峰值+25.9%),基金线触发+10.90%",
                    "微盘股崩盘尾声(上证-1.5%)入场QDII冠军;75天缓慢盘整,"
                    "基金线触发仅+0.43%——境外资产与A股脱钩,超额收益有限",
