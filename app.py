        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            st.dataframe(pd.DataFrame({
                "买入日": ["2018-02-09", "2018-10-29", "2019-02-25",
                          ...
                          "2024-10-09", "2025-04-07"],
                "当时近3月冠军(C类全市场,剔除海外/赎回假象)": [
                    "工银新趋势灵活配置C (001997)",
                    "长安鑫益增强混合C (002147)",
                    ...
                    "中欧北证50成份指数C (021299)",
                    "鹏华碳中和主题混合C (016531)"],
                "类型": ["混合型-灵活", "混合型-偏债", "指数型-股票(券商)",
                        "混合型-偏股", "混合型-偏股", "混合型-偏股",
                        "混合型-灵活", "指数型-股票(煤炭)",
                        "指数型-股票(券商)", "指数型-股票(北证50)",
                        "混合型-偏股(机器人)"],
                "冠军近3月涨幅": ["+8.88%", "+4.78%", "+37.12%", "+35.57%",
                                "+34.36%", "+26.76%", "+44.60%", "+13.05%",
                                "+43.74%", "+59.23%", "+40.52%"],
                "卖出日(回撤>5%)": ["2018-03-23", "2018-11-29", "2019-04-26",
                                  "2020-02-28", "2020-03-18", "2020-07-16",
                                  "2020-09-09", "2022-07-15", "2024-10-09",
                                  "2024-11-22", "2025-12-16"],
                "持有收益": ["-4.51%", "+1.71%", "-1.21%", "+22.11%",
                            "-4.69%", "+32.52%", "-2.68%", "+10.34%",
                            "+6.05%", "+33.24%", "+31.09%"],
                "同期上证": ["+0.7%", "+1.0%", "+4.2%", "+4.8%",
                            "-6.6%", "+17.6%", "+1.4%", "+10.2%", "-2.3%",
                            "+0.2%", "+23.5%"],
                "点评": [
                    "恐慌坑底但价值动量已死:随后风格切向成长,大盘没跌冠军先亏",
                    "熊市:债基/固收+霸榜本身就是防守信号,顺势买固收冠军小赚"
                    "(硬选权益冠军银行指数是 -2.5%)",
                    "券商涨停潮亢奋日追入,动量透支,牛市里跑输大盘 5 个点",
                    "恐慌一步到位(大盘 -7.7%)+科技动量未完,19 个交易日 +22%"
                    "(峰值 +34%)",
                    "熔断中段恐慌未尽,4 个交易日止损离场——躲开了更深的坑",
                    "熔断尾声抄底,同一只冠军拿满 120 天(峰值 +51.6%)",
                    "垂直拉升后追入;当日急跌 -8.9% 的白酒冠军(仅有A类)反而"
                    " +7.1%,恐慌折价的价值",
                    "恐慌日(-5.1%)买国内冠军;原始榜首原油 QDII 与 A 股反弹"
                    "脱钩仅 +1.3%",
                    "亢奋日买入,吃到 10/8 最后一冲后止损快跑,运气钱不可复制",
                    "恐慌日(-6.6%)买动量最强冠军,全表最佳;10/11 一度浮亏 7%,"
                    "大盘止损未触发故拿住",
                    "关税暴跌日(-7.3%)买入,冠军当天自身 -14.1% 折价拉满;拿了"
                    " 253 天峰值 +62.5%,9 月动量见顶后回吐近半——大盘 5% 线对"
                    "高贝塔冠军离场偏慢,但仍 +31%",
                ],
            ...
            st.caption(
                ...
                "赎回费入账)。三步规则:恐慌急跌日触发(QVIX>30 且当日下跌,"
                "或 -5% 级暴跌)→ 看榜单结构定攻防(权益霸榜买权益冠军,债基"
                "霸榜买固收冠军)→ 大盘从买入后高点回撤 5% 离场。")
