...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # pct=0.90) + 候选按近3月跌幅从低到高排(ret_col="ret_3m",
        ...
        # 拥挤的仓位, 跌得最惨的反而容易报复性反弹)。相关系数过滤
        # (min_corr, 之前用2年90%+涨幅冠军定过0.6)对这条新逻辑没有
        # 实测过, 已关闭。此前"2年90%+涨幅冠军+相关系数≥0.6"的11笔
        # 复盘记录已被替换, 历史版本见 git 历史。
        #
        ...
        # 脏数据)。候选池剔除:QDII/海外指数型(跟踪境外市场,与 QVIX/大盘
        # 恐慌-反弹逻辑脱钩)、有持有期锁定的基金(名称含"N年/N个月/N天
        ...
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月跌幅最大"
            ...
            st.caption("12 笔已完成:胜率 66.7%(8/12),费后复利 +410.14%,"
                       "累计手续费 2.0%,平均持有 56 天,平均费后 +18.71%,"
                       "最佳 +140.23% / 最差 -4.81%。"
                       "注:+140.23%那笔(021528财通成长优选混合C)是单只"
                       "小微盘主题基金的极端案例,剔除它其余11笔累计约"
                       "+112%,别把140%当期望值——但亏损普遍很小这一点"
                       "不依赖这笔运气,是结构性特征:候选池普遍波动率"
                       "比值低、回撤线天然更窄,且已经跌透、下跌空间"
                       "有限,最大亏损仅-4.81%,平均亏损-1.72%,明显小于"
                       "旧版\"涨幅冠军\"策略的-9.40%/-5.37%。"
                       "橙色=触发基金回撤线离场,蓝色=触发大盘回撤线离场;"
                       "连续接力同一只基金视为未真实离场,中间腿不收手续费,"
                       "只在链条最后一腿按累计持有天数收一次。")
            _review_trigger = [
                "sse", "fund", "sse", "sse", "sse", "fund", "fund", "fund",
                "fund", "fund", "fund", "sse",
            ]
            _review_df = pd.DataFrame({
                ...
                "类型": ["混合型-灵活", "混合型-灵活", "混合型-灵活",
                        "指数型-股票", "混合型-灵活", "混合型-偏股",
                        "混合型-灵活", "股票型", "混合型-灵活",
                        "混合型-偏股", "混合型-灵活", "混合型-偏股"],
                "波动率比值(近3月)": [0.16, 0.12, 1.73, 2.09, 3.07,
                                   1.89, 1.93, 1.10, 0.90, 1.10,
                                   3.41, 2.34],
                "恐慌阈值": [29.11, 29.60, 26.33, 23.24, 22.28,
                            22.28, 20.45, 20.51, 21.75, 21.77,
                            21.42, 22.54],
                "回撤控制线(%)": [0.93, 0.71, 9.11, 9.71, 13.68,
                                8.42, 7.89, 4.51, 3.92, 4.79,
                                14.61, 10.55],
                "大盘回撤线(%)": [5.82, 5.92, 5.27, 4.65, 4.46,
                                4.46, 4.09, 4.10, 4.35, 4.35,
                                4.28, 4.51],
                "近3月跌幅(前日口径)": [
                    "-1.13%", "-3.15%", "-37.80%", "-27.01%", "-31.63%",
                    "-36.76%", "-22.41%", "-5.99%", "-12.40%", "-5.21%",
                    "-22.35%", "-20.75%"],
                "卖出日": ["2020-07-16", "2020-09-04",
                          "2022-07-15", "2022-12-22", "2023-10-19",
                          "2024-03-27", "2024-10-09", "2024-10-31",
                          "2024-11-14", "2024-12-17",
                          "2025-11-18", "2026-05-21"],
                "持有收益": ["-0.06% (费后-0.56%)", "+0.55%", "+14.19%",
                            "+31.54%", "-0.77%", "+23.24%",
                            "+18.05% (费后+17.55%)", "-0.26% (费后-0.76%)",
                            "+1.49% (费后+0.99%)", "-4.81%",
                            "+140.23%", "+3.10%"],
                "期间最高": ["+0.0%", "+1.3%", "+23.2%", "+35.8%", "+7.2%",
                            "+36.3%", "+32.8%", "+5.7%", "+6.5%", "+0.5%",
                            "+181.4%", "+17.1%"],
                "期间最大回撤": ["0.1%", "0.8%", "7.3%", "8.7%", "10.7%",
                              "9.6%", "11.1%", "5.6%", "4.7%", "5.3%",
                              "14.6%", "11.9%"],
                "同期上证": ["-3.7%", "+4.5%", "+10.2%", "+2.6%", "-3.0%",
                            "+10.8%", "+8.6%", "+0.6%", "+3.0%", "-0.5%",
                            "+27.2%", "+6.9%"],
                "备注": [
                    "近3月跌幅温和(-1.13%,全表最浅之一);10天后大盘线"
                    "触发,-0.56%(费后)小亏",
                    "近3月跌幅-3.15%;波动率比值仅0.12(全表最低),回撤线"
                    "天然窄(0.71%);50天基金线触发,+0.55%",
                    "近3月跌幅-37.80%(全表最深);81天大盘线触发,+14.19%,"
                    "超跌反弹兑现",
                    "港股互联网超跌反弹段入场(近3月跌幅-27.01%,与同期"
                    "港股互联网强势反弹重合,见前面\"22年10月涨幅最好的"
                    "基金\"分析);59天大盘线触发,+31.54%",
                    "近3月跌幅-31.63%;52天大盘线触发,-0.77%小亏——超跌"
                    "不代表一定反弹",
                    "雪球产品集中平仓踩踏后入场,近3月跌幅-36.76%;51天"
                    "基金线触发,+23.24%",
                    "924行情前夕入场,近3月跌幅-22.41%;13天基金线触发,"
                    "+18.05%(费后+17.55%)",
                    "近3月跌幅最浅的一笔(-5.99%,波动率比值1.10);22天"
                    "基金线触发,-0.26%(费后-0.76%)小亏",
                    "近3月跌幅-12.40%;14天基金线触发,+1.49%(费后"
                    "+0.99%)",
                    "近3月跌幅-5.21%;33天基金线触发,-4.81%,全表最差,"
                    "但亏损幅度仍明显小于旧版冠军策略的最差-9.40%",
