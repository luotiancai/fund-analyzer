...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # ── 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘(backtest_qvix.py) ──
        # 结果为脚本离线跑批后硬编码(全市场近3月涨幅重算无法在页面现算)。
        # 冠军排名复用 fetcher.compute_metrics_asof——与本页「基金列表」的
        # "截至日期"筛选完全同口径(按日收益率连乘计算区间收益,正确处理
        # 分红除权,自带单日|收益率|>30%异常值过滤),严格早于当日截断(T日
        # 决策只能看到T-1日收盘净值)。此前脚本自算的 end_nav/anchor_nav-1
        # 是简化版,不处理分红、口径与本页籍不一致(如 2020-07-16 曾算成
        # 广发医疗保健+58.95%夺冠,而「基金列表」按同一天筛选显示汇丰晋信
        # 智造先锋才是真冠军57.94%,已核实并切换到统一口径重算)。
        # 止损逐交易日检查,双线在买入日锁定——基金线=阈值/5×波动率比值,
        # 大盘线=阈值/5;波动率比值=基金日收益率std/大盘日收益率std(纯波动
        # 对比,不按相关系数加权,且剔除单日|收益率|>20%的净值重置/份额折算
        # 脏数据)。冠军候选池剔除 QDII(跟踪境外市场,与 QVIX/大盘恐慌-反弹
        # 逻辑脱钩,选入会削弱大盘回撤线对该笔仓位的意义)。连续接力同一只
        # 基金(上一腿卖出日=下一腿买入日且代码相同)视为未真实离场,中间腿
        # 不收手续费,只在链条最后一腿按"链条首次买入→该腿卖出"的累计持有
        # 天数收一次(而非按单腿天数)。冠军候选池排除"净值僵化-补涨"基金:
        # 排名窗口内连续≥5个交易日|日收益率|<0.05%(不像有股票仓位的基金
        # 该有的波动),紧接单日|收益率|>8%补涨/补跌——判断为净值长期未按
        # 市值更新、事后集中补记,而非真实动量(如 002631 江信瑞福灵活配置
        # 混合C 2024-02-02 排名窗口内曾以此模式虚高排进冠军, 直连东财源头
        # 核对非缓存问题, 但该基金全历史波动率其实正常, 原因不明; 剔除后
        # 真实冠军为万家精选混合C, 与本仓库最早一版手工整理的复盘表一致,
        # 交叉验证)。此规则与 effective_daily_ret 的单日>30%硬过滤是两种
        # 不同场景, 互不替代。数据 2018 年起,2018 全年无信号日(滚动3年
        # 窗口含 2015 股灾,阈值 32~47 高于全年 QVIX 峰值 33.06)。
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("15 笔已完成:胜率 66.7%(10/15),费后复利 +323.56%,"
                       "累计手续费 2.0%,平均持有 57 天,平均费后 +11.02%,"
                       "最佳 +39.10% / 最差 -8.53%。"
                       "橙色=触发基金回撤线离场,蓝色=触发大盘回撤线离场;"
                       "连续接力同一只基金视为未真实离场,中间腿不收手续费,"
                       "只在链条最后一腿按累计持有天数收一次。")
            _review_trigger = [
                "sse", "sse", "fund", "sse", "fund", "fund", "fund", "fund",
                "sse", "fund", "fund", "fund", "sse", "sse", "sse",
            ]
            _review_df = pd.DataFrame({
                "买入日": ["2019-02-25", "2019-05-06",
                          "2020-02-03", "2020-03-12", "2020-03-18",
                          "2020-07-16", "2022-04-25", "2024-02-02",
                          "2024-09-27", "2024-10-09", "2024-10-11",
                          "2024-10-22", "2024-11-01",
                ...
                "冠军(C类全市场,按前一交易日榜单)": [
                    ...
                    "鹏华碳中和主题混合C (016531)",
                    "广发远见智选混合C (016874)"],
                "类型": ["混合型-灵活", "混合型-灵活", "指数型-股票",
                        "混合型-偏股", "混合型-偏股", "股票型",
                        "指数型-股票", "混合型-偏股", "混合型-灵活",
                        "指数型-股票", "指数型-股票", "指数型-股票",
                        "指数型-股票", "混合型-偏股",
                        "混合型-偏股"],
                "波动率比值(近3月)": [1.33, 1.76, 2.71, 1.61, 1.63, 1.85,
                                   1.84, 1.64, 2.35, 1.40, 1.19, 1.69, 1.83,
                                   4.88, 3.82],
                "恐慌阈值": [28.29, 29.37, 29.41, 29.41, 29.64, 30.93,
                            29.71, 23.47, 23.11, 23.26, 23.33, 23.80, 24.54,
                            25.01, 24.88],
                "回撤控制线(%)": [7.53, 10.34, 15.94, 9.47, 9.66, 11.45,
                                10.93, 7.70, 10.86, 6.51, 5.55, 8.04, 8.98,
                                24.41, 19.01],
                "大盘回撤线(%)": [5.66, 5.87, 5.88, 5.88, 5.93, 6.19,
                                5.94, 4.69, 4.62, 4.65, 4.67, 4.76, 4.91,
                                5.00, 4.98],
                "冠军近3月涨幅(前日口径)": [
                    "+27.45%", "+52.40%", "+51.10%", "+38.29%", "+28.48%",
                    "+57.94%", "+14.33%", "+13.61%", "+25.77%", "+63.61%",
                    "+53.41%", "+93.61%", "+97.91%", "+54.70%", "+57.21%"],
                "卖出日": ["2019-04-29", "2019-08-05", "2020-03-09",
                          "2020-03-18", "2020-07-16", "2020-09-09",
                          "2022-06-22", "2024-03-18", "2024-10-09",
                          "2024-10-11", "2024-10-22", "2024-11-01",
                          "2024-11-22", "2025-12-16", "2026-06-05"],
                "持有收益": ["+10.75%", "-1.36%", "+6.93%",
                            "-4.68%", "+32.49%", "-3.20%",
                            "+10.90%", "+9.24%", "+3.03% (费后+2.53%)",
                            "-7.03% (费后-8.53%)", "+29.17%",
                            "-0.68%", "+11.62%",
                            "+31.06%", "+39.10%"],
                "期间最高": ["+18.0%", "+6.8%", "+36.5%", "+0.7%", "+51.6%",
                            "+10.5%", "+25.9%", "+18.4%", "+9.5%",
                            "+0.0%", "+39.4%", "+13.1%", "+20.6%", "+62.5%",
                            "+50.6%"],
                "期间最大回撤": ["6.3%", "7.7%", "21.7%", "5.7%", "12.6%",
                              "12.4%", "11.9%", "7.7%", "5.9%",
                              "7.0%", "7.3%", "12.2%", "7.5%", "21.4%",
                              "12.6%"],
                "同期上证": ["+3.4%", "-2.9%", "+7.2%", "-6.7%", "+17.6%",
                            "+1.4%", "+11.6%", "+13.0%", "+5.5%",
                            "-1.3%", "+2.1%", "-0.4%", "-0.1%", "+23.5%",
                            "+5.6%"],
                "备注": [
                    "QVIX破阈值当天上证大涨+5.6%(亢奋型触发,非恐慌下跌);"
                    "63天后大盘线先触发,+10.75%落袋(峰值+18.0%)",
                    "贸易战暴跌日(上证-5.6%)入场;91天横盘后大盘线触发,"
                    "-1.36%小亏",
                    "春节后熔断式暴跌(上证-7.7%)入场半导体主题(波动率比值"
                    "2.71×);35天冲高至+36.5%后随疫情扩散回落,基金线触发"
                    "+6.93%落袋",
                    "全球熔断潮初期(上证-1.5%)入场;6天内被二次探底砸中,"
                    "大盘线先触发,-4.68%——当天原地接回同一冠军(下一条),"
                    "视为未真实离场不收手续费",
                    "熔断尾声抄底同一冠军(第二次入场);120天拿满整轮V型反弹"
                    "(峰值+51.6%),基金线触发离场仍+32.49%",
                    "\"疯牛\"回调日(上证-4.5%)入场;55天基金线(11.4%线)触发,"
                    "-3.20%小亏——冠军动量未能延续到止损前",
                    "上海封控恐慌底(上证-5.1%)入场;58天吃到煤炭超级周期一段"
                    "(峰值+25.9%),基金线触发+10.90%",
                    "微盘股崩盘尾声(上证-1.5%)入场;原始榜首江信瑞福灵活配置"
                    "混合C 排名窗口内连续多日净值几乎零波动、随后单日补涨"
                    "15.7%(净值僵化-补涨模式,已核实剔除),真实冠军为本基金"
                    "(+13.61%);45天基金线触发,+9.24%落袋(峰值+18.4%)",
                    "924行情亢奋周(上证+2.9%,非恐慌)追入;12天大盘线触发,"
                    "+3.03%(费后+2.53%)",
                    "924行情后首个大跌日(上证-6.6%)追高近3月+63.6%的北证50"
                    "冠军;2天基金线急速触发,费后-8.53%——全表最差,追高反转"
                    "最快的一笔",
                    "上证仍在回调(上证-2.6%)入场;11天基金线快速触发,"
                    "+29.17%——当天原地接回同一冠军(下一条),未真实离场不收"
                    "手续费",
                    "同一冠军(北证50)再度登顶,近3月涨幅已冲到+93.6%;10天"
                    "基金线触发,-0.68%——追第二棒动量到顶回落,当天原地接回"
                    "第三棒不收手续费",
                    "第三次连续接力同一冠军;21天后大盘线触发,+11.62%——三棒"
                    "连续接力的链条(10/11→11/22共42天)按累计持有天数只在"
                    "此收一次手续费,≥30天费率为0",
                    "\"对等关税\"暴跌日(上证-7.3%,近年单日最大跌幅之一)入场;"
                    "原始榜首同泰产业升级混合C 近3月涨幅含一笔无法解释的"
                    "净值跳变(已核实非真实收益),真实冠军为本基金(+54.70%);"
                    "253天基金线(24.41%宽线)一路扛住震荡未曾止损,直到大盘线"
                    "触发,+31.06%——期间原本该空仓的窗口被这笔长仓占满,"
                    "后面一笔科技主题冠军因此没有入场机会",
                    "上证单日-3.6%入场;74天大盘线触发离场,+39.10%——全表"
                    "最佳单笔收益",
                ],
            })
