...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            st.dataframe(pd.DataFrame({
                "买入日": ["2018-02-09", "2018-10-29", "2019-02-25", "2020-02-03"],
                "当时近3月冠军(剔除赎回假象)": [
                    "工银新趋势灵活配置A (001716)",
                    "博时裕瑞纯债 (001578)",
            ...
            st.caption(
                ...
                "日 +22%(峰值 +34%)。结论:追冠军的成败=买入时点的恐慌程度 ×"
                "冠军风格的剩余动量;垂直拉升后追入必败,恐慌坑底追入尚有动量的"
                "冠军才是正确姿势。2018 两次榜单原始前几名均为巨额赎回假净值"
                "(单日 +8%~+19% 的\"纯债\"),已剔除;2019/2020 榜首真实。")
