    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            ...
            st.caption(
                ...
                "+7.1%、峰值 +17.8%,跑赢同期上证近 6 个点。案例八限定国内资产"
                "冠军(原始榜首是国泰大宗商品等原油 QDII,动量在海外油价、与"
                "A 股恐慌修复脱钩,持有仅 +1.3%;剔除 QDII 后冠军为煤炭仓的"
                "万家宏观择时):恐慌日 -5.1% 买入,81 天 +14.5%、峰值 +26.3%,"
                "跑赢 V 弹的大盘——恐慌买点必须配国内资产冠军。2018 两次榜单"
                "原始前几名均为巨额赎回假净值(单日 +8%~+19% 的\"纯债\"),"
                "已剔除;2019/2020/2022 榜首真实。")
