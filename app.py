...
with tab_gold:
    ...
    if au_df is None or au_df.empty:
        ...
        # ── 黄金离场规则复盘:高点回撤15%卖出 ────────────────────────────────
        # 结论为离线回测硬编码(Au99.99 全史 2017-2026,复盘于 2026-07):
        # 回撤分布天然分两档——正常洗盘 5%~12%(8次),真顶部 20%+(2次),
        # 13%~19% 是空档;15% 线卡在"洗盘够不到、真顶部必触发"的位置,
        # 与基金 20% 线的定线逻辑一致(线设在日常洗盘振幅上沿之上)。
        with st.expander("📜 黄金离场规则:高点回撤15%卖出(定线依据)"):
