...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # 天数收一次(而非按单腿天数)。数据 2018 年起,2018 全年无信号日
        # (滚动3年窗口含 2015 股灾,阈值 32~47 高于全年 QVIX 峰值 33.06)。
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("17 笔已完成:胜率 58.8%(10/17),费后复利 +262.13%,"
                       "累计手续费 3.0%,平均持有 50 天,平均费后 +8.75%,"
                       "最佳 +39.10% / 最差 -11.38%。"
            ...
            _review_df = pd.DataFrame({
                "买入日": ["2019-02-25", "2019-05-06",
                          ...
                          "2025-04-07", "2025-08-25", "2026-03-23"],
                "冠军(C类全市场,按前一交易日榜单)": [
                    "前海开源价值成长混合C (006217)",
                ...
                "冠军近3月涨幅(前日口径)": [
                    ...
                    "+57.62%", "+58.24%", "+102.69%", "+98.98%", "+88.76%",
                    "+108.53%", "+52.12%"],
                "卖出日": ["2019-04-29", "2019-08-05", "2020-03-09",
                          ...
                          "2024-11-01", "2024-11-22", "2025-06-13",
                ...
                "备注": [
                    ...
                    "67天基金线(13.4%宽线扛住关税后剧烈震荡)触发,+7.95%"
                    "落袋",
                    "科技股非恐慌上涨日(上证+1.5%,亢奋型触发)追入近3月"
                    "+108.5%冠军(波动率比值5.62×,全表最高);113天大盘线"
                    "触发,+30.14%——稳稳吃到科技主题下半年行情",
