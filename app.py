...
with tab_gold:
    ...
    if au_df is None or au_df.empty:
        ...
        with st.expander("📜 黄金策略复盘:高点回撤15%卖出"):
            st.dataframe(pd.DataFrame({
                "买入日": ["2019-06-24", "2022-03-07"],
                "买入价(元/克)": ["313.26", "401.50"],
                "卖出日": ["2020-11-25", "2026-03-23"],
                "卖出价(元/克)": ["378.45", "924.65"],
                "持有收益": ["+20.8%", "+130.3%"],
                "持有天数": ["520", "1477"],
                "期间最高": ["+42.4% (2020-08-07)", "+209.6% (2026-01-29)"],
                "期间最大回撤": ["15.1% (触发线)", "25.6% (含二次监听段)"],
                "点评": [
                    ...
                    "牛市;2026-01-29 见顶 +209.6% 后两日暴跌 17%,02-02 触线"
                    "但当日 GVZ 37.4 远超阈值 24.5(恐慌急跌不卖),重置高点续"
                    "监听;V 弹至 03-02 的 1199 未创新高,03-23 二次触线离场"
                    "落袋 +130.3%——恰逢 A 股 QVIX 冲 42 的恐慌买点日,离场"
                    "资金无缝衔接权益冠军"],
            }), use_container_width=True, hide_index=True)
            st.caption(
