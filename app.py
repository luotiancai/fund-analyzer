        ...
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月跌幅最大"
            ...
            if _bt_df is None or _bt_df.empty:
                ...
                st.caption(
                    ...
                    "「同池分位」是这一笔在当天同口径候选池(规模≥2亿、排除QDII/海外"
                    "和持有期锁定和净值脏数据,一两千只)里排第几——分母里每只都按同一套"
                    "双止损跑过,50%就等于闭眼瞎选;本表8笔平均88.1%。"
                ...
                _review_df = _review_df[[c for c in [
                    ...
                    # 「同池分位」= 这一笔在**当天同口径候选池**里排第几(打败了
                    # 百分之多少)。分母跟「可达天花板」那条对照跑批一致: 规模≥2亿
                    # (与标准策略同口径)、排除 QDII/海外和持有期锁定和净值脏数据,
                    # 其余不设条件; 每只候选都按同一套双止损跑出实现收益, 跟本笔
                    # 可比; 50% 就等于闭眼瞎选。
