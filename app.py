...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,大盘回撤超5%卖出"):
            # 5% 定线依据(2026-07 校准,别随手改):12 次 B 点触发按单份资金
            # 串行回测,固定线 3%~10% 网格中 5% 最优(+379.5%、12 次全参与,
            # 4%~6% 为稳健平台;7%+ 因持仓重叠错过下次 B 点砍到 +217%)。
            # 波动率口径自洽(近10年窗口):线 = 5×近10年日σ = 5×1.03% ≈ 5%,
            # 交叉验证 QVIX 近10年中位 18.72÷4 = 4.7%;上证常态σ按 1/3/10 年
            # 窗口为 0.81/0.79/0.82,中枢稳定故窗口不敏感。断裂检测:近1年σ÷
            # 近10年σ>2 即时代迁移需重算(KOSPI 2026 为 2.2 倍的反例)。
            # 动态线(QVIX/实际σ × 入场定死/每日跟随,k=2~6)无稳健增益——
            # B 点本身是恐慌日(QVIX 24~43),按入场波动率定线永远给宽线,
            ...
            st.dataframe(pd.DataFrame({
                ...
                "冠军近3月涨幅(前日口径)": [
                    ...
                    "+25.77%", "+61.40%", "+63.60%", "+47.23%"],
                "卖出日": ["2019-04-26", "2019-07-22",
                          "2020-02-28", "2020-03-18", "2020-07-16",
                          "2020-09-09", "2022-07-15", "2024-06-20", "2024-10-09",
                          "2024-11-22", "2025-11-20 (基金20%线)",
                          "2026-06-05"],
                "持有收益": ["+10.69%", "+0.95%",
                            "+22.11% (费后+21.50%)",
                            "-12.05% (费后-13.37%)",
                            "+32.52%", "-3.17%", "+10.34%", "+15.92%",
                            "+3.04% (费后+2.52%)",
                            "+33.24%", "+27.74%", "+39.08%"],
                "期间最高": ["+18.1%", "+6.8%", "+34.2%",
                            "+0.0%", "+51.6%", "+10.5%", "+27.6%", "+21.7%",
                            "+9.5%", "+43.9%", "+62.5%", "+50.6%"],
                "期间最大回撤": ["6.3%", "6.7%", "9.0%",
                              "12.1%", "12.6%", "12.4%", "13.5%", "10.2%",
                              "5.9%", "12.2%", "21.4%", "12.6%"],
                "同期上证": ["+4.2%", "-0.7%", "+4.8%",
                            "-6.6%", "+17.6%", "+1.4%", "+10.2%", "+10.1%", "+5.5%",
                            "+0.2%", "+26.9%", "+5.6%"],
                "点评": [
                    "前日口径避开了涨停当天才冲上榜首的券商(那笔是 -1.2%),"
                    "买到均衡成长冠军,4 月冲顶 +18% 后落袋 +10.7%",
                    "贸易战暴跌日(-5.6%),QVIX 29.41 险破阈值 29.37 的边缘触发;"
                    "恐慌后没有 V 弹而是阴跌到 8 月,冠军横盘 77 天 +0.95% 小胜"
                    "大盘——边缘触发对应边缘收益",
                    "恐慌一步到位(大盘 -7.7%)+科技动量未完,19 个交易日 +22%"
                    "(峰值 +34%)",
                    "熔断中段接高贝塔飞刀,4 个交易日 -12%——止损是唯一的救赎",
                    "熔断尾声抄底,冠军拿满 120 天(峰值 +51.6%)",
                    "垂直拉升后追入,峰值 +10.5% 未兑现回落成亏",
                    "恐慌日(-5.1%)买国内冠军;原始榜首原油 QDII 与 A 股反弹"
                    "脱钩仅 +1.3%",
                    "微盘股崩盘尾声(QVIX 23.72 险破阈值 23.47),榜首为赎回假象,剔除"
                    "后买到煤炭价值冠军,139 天 +15.9% 跑赢大盘 6 个点",
                    "亢奋周的周五触发(+2.9% 上涨日,QVIX 28.56 破阈值 23.11),"
                    "吃到 9/30 +7.5% 后冠军熄火,10/9 止损;费后 +2.5% 跑输同期"
                    "大盘——亢奋型触发连超额都没有",
                    "恐慌日(-6.6%)买动量最强冠军;10/11 一度浮亏 7%,大盘止损"
                    "未触发故拿住",
                    "关税暴跌日(-7.3%)买入,冠军当天自身 -14.1% 折价拉满;峰值"
                    " +62.5% 后回吐,11/20 基金 20% 线先于大盘 5% 线触发离场"
                    "(10%/15% 线会在 5-6 月洗盘中被抖出局,仅 +8%~11%)",
                    "QVIX 冲 42 + 单日 -3.6% 的恐慌底,冠军动量延续到 5 月顶,"
                    "+39.1% 全表最佳;6/5 止损离场躲开 6-7 月的 -11% 大跌",
                ],
            }), use_container_width=True, hide_index=True)

        # ── 回测对比:同规则逐日双止损(backtest_qvix.py 2026-07 重算) ──────────
        # 与上表的差异:止损逐交易日检查(上表复盘口径为信号日检查),双线在
        # 买入日锁定——基金线=阈值/5×Beta,大盘线=阈值/5;数据 2018 年起,
        # 2018 全年无信号日(滚动3年窗口含 2015 股灾,阈值 32~47 高于全年
        # QVIX 峰值 33.06)。结果为脚本离线跑批后硬编码,与上表对比用。
        with st.expander("📊 回测对比:同规则逐日双止损(2018起,脚本重算)"):
            st.caption("19 笔已完成:胜率 68.4%(13/19),费后复利 +252.32%,"
                       "累计手续费 10.5%,平均持有 28 天,平均费后 +7.47%,"
                       "最佳 +31.62% / 最差 -11.38%。"
                       "与上表区别:止损逐日盯盘、双线买入日锁定,故止损更早、"
                       "笔数更多(19 vs 12)、单笔持有更短。")
            st.dataframe(pd.DataFrame({
                "买入日": ["2019-02-25", "2019-03-14", "2019-05-06",
                          "2020-02-03", "2020-03-12", "2020-03-18",
                          "2020-03-23", "2020-07-06", "2020-07-16",
                          "2020-07-24", "2022-04-25", "2024-02-02",
                          "2024-09-27", "2024-10-09", "2024-10-15",
                          "2024-11-01", "2025-04-07", "2025-08-25",
                          "2026-03-23"],
                "冠军(C类全市场,按前一交易日榜单)": [
                    "前海开源价值成长混合C (006217)",
                    "前海开源价值成长混合C (006217)",
                    ...
                    "永赢科技智选混合发起C (022365)",
                    "广发远见智选混合C (016874)"],
                "类型": ["混合型-灵活", "混合型-灵活", "混合型-灵活",
                        "指数型-股票", "混合型-偏股", "混合型-偏股",
                        "混合型-偏股", "股票型", "股票型", "股票型",
                        "指数型-股票", "QDII-混合灵活", "指数型-股票",
                        "指数型-股票", "指数型-股票", "指数型-股票",
                        "混合型-偏股", "混合型-偏股", "混合型-偏股"],
                "Beta(近3月)": [0.68, 0.92, 1.27, 1.01, 0.97, 1.06, 1.02,
                               1.85, 0.10, 1.12, 1.10, 0.16, 1.02, 1.51,
                               1.91, 1.87, 0.77, 0.74, 2.39],
                "恐慌阈值": [28.29, 28.19, 29.37, 29.41, 29.41, 29.64, 30.00,
                            30.50, 30.93, 30.98, 29.71, 23.47, 23.11, 23.26,
                            23.39, 24.54, 25.01, 24.88, 24.88],
                "回撤控制线(%)": [3.85, 5.19, 7.46, 5.94, 5.71, 6.28, 6.12,
                                11.29, 0.62, 6.94, 6.54, 0.75, 4.71, 7.02,
                                8.94, 9.18, 3.85, 3.68, 11.89],
                "大盘回撤线(%)": [5.66, 5.64, 5.87, 5.88, 5.88, 5.93, 6.00,
                                6.10, 6.19, 6.20, 5.94, 4.69, 4.62, 4.65,
                                4.68, 4.91, 5.00, 4.98, 4.98],
                "冠军近3月涨幅(前日口径)": [
                    "+22.12%", "+41.97%", "+51.68%", "+48.90%", "+51.96%",
                    "+34.91%", "+28.81%", "+49.54%", "+58.95%", "+59.91%",
                    "+20.18%", "+20.79%", "+23.20%", "+57.62%", "+58.24%",
                    "+98.98%", "+88.76%", "+108.53%", "+52.12%"],
                "卖出日": ["2019-03-14", "2019-04-25", "2019-08-05",
                          "2020-02-27", "2020-03-18", "2020-03-23",
                          "2020-05-22", "2020-07-16", "2020-07-20",
                          "2020-08-20", "2022-06-20", "2024-02-20",
                          "2024-10-09", "2024-10-15", "2024-11-01",
                          "2024-11-22", "2025-05-15", "2025-09-02",
                          "2026-05-27"],
                "持有收益": ["+8.30% (费后+7.80%)", "+2.66%", "-1.36%",
                            "+24.78% (费后+24.28%)", "-4.68% (费后-6.18%)",
                            "-6.42% (费后-7.92%)", "+18.26%",
                            "-0.20% (费后-0.70%)", "+0.50% (费后-1.00%)",
                            "+1.62% (费后+1.12%)", "+12.88%",
                            "+0.86% (费后+0.36%)", "+4.81% (费后+4.31%)",
                            "-9.88% (费后-11.38%)", "+22.87% (费后+22.37%)",
                            "+11.62% (费后+11.12%)", "+19.93%",
                            "+14.26% (费后+13.76%)", "+31.62%"],
                "期间最高": ["+13.9%", "+9.0%", "+6.8%", "+36.5%", "+0.7%",
                            "+0.6%", "+26.2%", "+11.2%", "+1.4%", "+9.6%",
                            "+25.9%", "+2.1%", "+15.4%", "+0.0%", "+39.9%",
                            "+20.6%", "+25.6%", "+19.6%", "+50.6%"],
                "期间最大回撤": ["4.9%", "5.8%", "7.7%", "8.6%", "5.7%",
                              "7.0%", "6.3%", "10.2%", "0.9%", "7.3%",
                              "10.4%", "1.2%", "9.1%", "9.9%", "12.2%",
                              "7.5%", "4.5%", "4.5%", "12.6%"],
                "同期上证": ["+1.0%", "+4.5%", "-2.9%", "+8.9%", "-6.7%",
                            "-2.5%", "+5.8%", "-3.7%", "+3.2%", "+5.2%",
                            "+13.2%", "+7.1%", "+5.5%", "-1.8%", "+2.2%",
                            "-0.1%", "+9.2%", "-0.7%", "+7.4%"],
                "卖出原因": ["基金4.9%≥3.8%", "基金5.8%≥5.2%", "基金7.7%≥7.5%",
                            "基金8.6%≥5.9%", "大盘6.7%≥5.9%", "基金7.0%≥6.3%",
                            "基金6.3%≥6.1%", "大盘7.0%≥6.1%", "基金0.9%≥0.6%",
                            "基金7.3%≥6.9%", "基金10.4%≥6.5%", "基金1.2%≥0.8%",
                            "基金9.1%≥4.7%", "基金9.9%≥7.0%", "基金12.2%≥8.9%",
                            "大盘5.9%≥4.9%", "基金4.5%≥3.9%", "基金4.5%≥3.7%",
                            "基金12.6%≥11.9%"],
            }), use_container_width=True, hide_index=True)
