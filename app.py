...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("15 笔已完成:胜率 60.0%(9/15),费后复利 +279.83%,"
                       "累计手续费 2.0%,平均持有 63 天,平均费后 +10.27%,"
                       "最佳 +39.10% / 最差 -8.53%。"
                       "橙色=触发基金回撤线离场,蓝色=触发大盘回撤线离场;"
                       "连续接力同一只基金视为未真实离场,中间腿不收手续费,"
                       "只在链条最后一腿按累计持有天数收一次。")
            _review_trigger = [
                "sse", "sse", "fund", "sse", "fund", "fund", "fund", "sse",
            ...
            _review_df = pd.DataFrame({
                ...
                "备注": [
                    ...
                    "(峰值+25.9%),基金线触发+10.90%",
                    "微盘股崩盘尾声(上证-1.5%)入场;136天基金线(10.8%线)"
                    "触发,-2.04%小亏——冠军全程未能明显突破买入价(峰值仅"
                    "+0.1%),横盘吃线出局",
