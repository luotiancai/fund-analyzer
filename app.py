...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # 只有9次真正满足条件,但质量明显更高。相关系数过滤(min_corr,
        # 中途试过0.5/0.6/0.7)最终判定对这套新逻辑没必要,已关闭。
        # 此前"涨幅冠军+相关系数≥0.6"及"跌幅最大但不设波动率/跌幅耗尽
        # 判定"的复盘记录均已替换, 历史版本见 git 历史。
        #
        ...
        # 脏数据)。候选池剔除:QDII/海外指数型、港股通/沪港深/恒生系列
        # (走港股通渠道不占QDII额度,type字段常不含"QDII"/"海外",实测
        # 漏过一只"国泰中证港股通科技ETF发起联接C",跟QDII同理排除——
        # 都是跟踪境外市场,与 QVIX/大盘恐慌-反弹逻辑脱钩)、有持有期锁定
        # 的基金(名称含"N年/N个月/N天持有(期)""滚动持有""定期开放"
        # "封闭式"等, 锁定期内实际赎不出来, 回测里的卖出是假的)、"净值
        # 僵化-补涨"基金(排名窗口内连续≥5个交易日|日收益率|<0.05%紧接
        # 单日|收益率|>8%补涨/补跌, 判断为净值长期未按市值更新、事后集中
        # 补记, 而非真实动量)。连续接力同一只基金(上一腿卖出日=下一腿
        # 买入日且代码相同)视为未真实离场,中间腿不收手续费,只在链条
        # 最后一腿按"链条首次买入→该腿卖出"的累计持有天数收一次(而非按
        # 单腿天数)。数据 2018 年起(2015-2017 期权刚上市流动性薄、QVIX
        # 计算噪声偏大, 已整段剔除, 见 fetcher.py)。
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月跌幅最大"
            ...
            st.caption("9 笔已完成:胜率 77.8%(7/9),费后复利 +317.69%,"
                       "累计手续费 1.0%,平均持有 63 天,平均费后 +22.35%,"
                       "最佳 +140.23% / 最差 -9.76%。"
                       "注:+140.23%那笔(021528财通成长优选混合C)是单只"
                       "小微盘主题基金的极端案例,剔除它其余8笔累计约"
                       "+73.87%,别把140%当期望值。79个QVIX信号日里只有"
                       "这9次同时满足\"真正下跌+波动率比值≥1.5\",其余全部"
                       "跳过不操作(2020年下半年那波尤其明显,QVIX反复触发"
                       "但从没凑出过合格候选)。"
                       "橙色=触发基金回撤线离场,蓝色=触发大盘回撤线离场;"
                       "连续接力同一只基金视为未真实离场,中间腿不收手续费,"
                       "只在链条最后一腿按累计持有天数收一次。")
            _review_trigger = [
                "sse", "fund", "fund", "sse", "fund", "fund", "fund",
                "fund", "fund",
            ]
            _review_df = pd.DataFrame({
                "买入日": ["2022-04-25", "2022-10-24", "2022-11-29",
                          "2023-08-28", "2024-02-05", "2024-09-26",
                          "2024-10-10", "2025-04-07", "2026-03-23"],
                "近3月跌幅最大标的(C类全市场,按前一交易日榜单)": [
                    ...
                    "财通成长优选混合C (021528)",
                    "东方城镇消费主题混合C (025884)"],
                "类型": ["混合型-灵活", "混合型-偏股", "混合型-灵活",
                        "混合型-灵活", "混合型-偏股", "混合型-灵活",
                        "混合型-偏股", "混合型-灵活", "混合型-偏股"],
                "波动率比值(近3月)": [1.73, 1.55, 2.13, 3.07, 1.89,
                                   1.93, 1.81, 3.41, 2.34],
                "恐慌阈值": [26.33, 23.24, 23.29, 22.28, 22.28,
                            20.45, 20.52, 21.42, 22.54],
                "回撤控制线(%)": [9.11, 7.20, 9.92, 13.68, 8.42,
                                7.89, 7.43, 14.61, 10.55],
                "大盘回撤线(%)": [5.27, 4.65, 4.66, 4.46, 4.46,
                                4.09, 4.10, 4.28, 4.51],
                "近3月跌幅(前日口径)": [
                    "-37.80%", "-24.17%", "-24.58%", "-31.63%", "-36.76%",
                    "-22.41%", "-2.42%", "-22.35%", "-20.75%"],
                "卖出日": ["2022-07-15", "2022-11-23", "2022-12-22",
                          "2023-10-19", "2024-03-27", "2024-10-09",
                          "2024-11-14", "2025-11-18", "2026-05-21"],
                "持有收益": ["+14.19%", "+6.74%", "-9.26% (费后-9.76%)",
                            "-0.77%", "+23.24%", "+18.05% (费后+17.55%)",
                            "+6.66%", "+140.23%", "+3.10%"],
                "期间最高": ["+23.2%", "+15.0%", "+2.2%", "+7.2%", "+36.3%",
                            "+32.8%", "+15.9%", "+181.4%", "+17.1%"],
                "期间最大回撤": ["7.3%", "7.2%", "11.2%", "10.7%", "9.6%",
                              "11.1%", "8.0%", "14.6%", "11.9%"],
                "同期上证": ["+10.2%", "+4.0%", "-3.0%", "-3.0%", "+10.8%",
                            "+8.6%", "+2.4%", "+27.2%", "+6.9%"],
                "备注": [
                    "近3月跌幅-37.80%(全表最深);81天大盘线触发,+14.19%,"
                    "超跌反弹兑现",
                    "港股互联网超跌反弹段附近入场(近3月跌幅-24.17%);"
                    "30天基金线触发,+6.74%——排除港股通基金后换成这只"
                    "境内标的,涨幅比未排除时选中的\"国泰中证港股通科技"
                    "ETF\"(+31.54%)弱不少,是排除规则的代价",
                    "上一笔卖出后腾出的窗口新触发一笔,近3月跌幅-24.58%;"
                    "23天基金线触发,-9.26%(费后-9.76%),全表最差",
