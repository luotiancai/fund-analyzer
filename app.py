    ...
    if au_df is None or au_df.empty:
        ...
        with st.expander("📜 黄金策略复盘:高点回撤15%卖出"):
            st.dataframe(pd.DataFrame({
                ...
                "买入价(元/克)": ["313.26", "401.50", "1030.00"],
                "卖出日": ["2020-11-25", "2026-02-02", "2026-03-23"],
                "卖出价(元/克)": ["378.45", "1030.00", "924.65"],
                "持有收益": ["+20.8%", "+156.5%", "-10.2%"],
                "持有天数": ["520", "1428", "49"],
                ...
                "点评": [
                    ...
                    " -11.2% 破线。当日 GVZ 43.4 仍超阈值 30.7,严格按规则应"
                    "再重置(那样将 06-11 @896 阈值下方触发离场,总收益"
                    " +123.2%);实际选择当日人工终止——恰逢 A 股 QVIX 冲 42"
                    " 的恐慌买点日,离场资金无缝衔接权益冠军。两段合计"
                    " +130.3%"],
            }), use_container_width=True, hide_index=True)
            st.caption(
                "口径:Au99.99 收盘价成交,自买入后最高收盘回撤≥15% 当日"
                "收盘卖出;例外:触发日 GVZ 若在滚动 3 年 95 分位阈值上方"
                "(恐慌性急跌,存在 V 弹可能),不卖,改从当日重置高点重新"
                "监听 15%,直至在阈值下方触发才真正离场(2022 案例:02-02"
                " 首触不卖 → 03-23 二次触发离场,少赚 26 个点但保留了创新高"
                "继续持有的可能)。无恐慌买点信号,入场由人工判断,GVZ 只作"
                "波动参考。定线依据(2026-07 回测):2017-2026 十轮回撤天然"
                "分两档——正常洗盘 5%~12%,真顶部 20%+;15% 取空档稳健位,"
                "洗盘够不到、真顶部必触发。")
