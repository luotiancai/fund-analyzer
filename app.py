        ...
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月跌幅最大"
            ...
            if _bt_df is None or _bt_df.empty:
                ...
                st.caption(
                    ...
                    "「全市场分位」是这一笔在当天全市场(几千只,只排除QDII/海外、"
                    "持有期锁定、净值脏数据)里排第几——分母里每只都按同一套双止损"
                    "跑过,50%就等于闭眼瞎选;本表8笔平均88.0%,同期\"每次买池中位数"
                    "那只\"累计只有+89.91%。"
                ...
                _review_df = _review_df[[c for c in [
                    ...
                    # 「全市场分位」= 这一笔在**当天全市场能买的基金**里排第几
                    # (打败了百分之多少)。分母是几千只候选各自跑完自身双止损后
                    # 的实现收益, 跟本笔同口径可比; 50% 就等于闭眼瞎选。由
                    # pct_rank.py 单独算好写进 trades —— 每个信号日要给两三千到
                    # 近五千只基金逐日跑止损, 约15分钟, 不适合塞进每次回测。
                    # ⚠️ 换了标准策略要重跑 pct_rank.py, 否则这列是上一版的旧数。
                    "卖出日", "持有收益", "全市场分位", "期间最高", "期间最大回撤",
                    "同期上证",
