...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘",
            ...
            st.caption("12 笔已完成:胜率 75.0%(9/12),费后复利 +342.69%,"
                       "累计手续费 0.5%,平均持有 71 天,平均费后 +14.22%,"
                       "最佳 +39.10% / 最差 -4.68%。"
            ...
            _review_df = pd.DataFrame({
                ...
                "恐慌阈值": [28.29, 29.37, 29.41, 29.41, 29.64, 30.93,
                            29.71, 23.47, 23.11, 23.26,
                            25.01, 24.88],
                "回撤控制线(%)": [7.53, 10.34, 15.94, 9.47, 9.66, 11.45,
                                10.93, 7.70, 10.86, 14.47,
                                24.41, 19.01],
                ...
                "备注": [
                    ...
                    "冠军;compute_beta 曾用>20%阈值把该基金9-30/10-8两天真实"
                    "+21.9%/+25.2%的补涨(北交所个股涨跌幅上限±30%,广发"
                    "017513同日+22.5%/+25.6%交叉验证非脏数据)当技术性跳变"
                    "剔除,波动率比值被压到1.19,回撤控制线收窄到6.5%,2天"
                    "内就被打出局,还被迫连续接回021299三段碎片仓位;阈值"
                    "改到35%后波动率比值回到3.11,回撤控制线拓宽到14.47%,"
                    "扛住10-11月震荡未再被打出,44天后大盘线触发,+36.90%"
                    "落袋——同一段行情,过滤器口径一变,策略表现从三段接力"
                    "的碎片化收益变成一段扛住波动的干净持仓",
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "冠军(C类全市场,按前一交易日榜单)": st.column_config.Column(width="medium"),
                    ...
                    "冠军近3月涨幅(前日口径)": st.column_config.Column(width="small"),
