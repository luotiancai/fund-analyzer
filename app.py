...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月跌幅最大"
                         "(波动率比值≥2.0 + 规模2~10亿 + 近3月跌幅≤30%;没有真跌的合格候选就"
                         "放弃当天、顺延到下一个信号日)"
                         " + 基金/大盘双止损逐日盯盘",
                          expanded=True):
                ...
                if _x_df is None or _x_df.empty:
                    ...
                    st.caption(
                        ...
                        + ((f"规模{_aum}~{_x_params['max_aum']}亿"
                            if (_aum, _x_params["max_aum"]) == (2.0, 10.0)
                            else f"规模**{_aum}~{_x_params['max_aum']}亿**")
