...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # 天数收一次(而非按单腿天数)。已核实净值异常修正:014939 同泰产业
        # 升级混合C 2025-03-31 单日+68.7%(直连东财源头核对非缓存问题,但
        # 前后走势平稳无回撤迹象,判断为净值断层而非真实收益),该跳变落在
        # 2025-04-07 的排名窗口内, 剔除后近3月涨幅从虚高的+88.76%降为
        # +11.87%,真实冠军应为鹏华碳中和主题混合C(+46.64%),已按此重算
        # (与该日期最早的手工复盘表一致,交叉验证)。数据 2018 年起,2018
        # 全年无信号日(滚动3年窗口含 2015 股灾,阈值 32~47 高于全年 QVIX
        # 峰值 33.06)。
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("16 笔已完成:胜率 56.2%(9/16),费后复利 +237.83%,"
                       "累计手续费 3.0%,平均持有 58 天,平均费后 +8.86%,"
                       "最佳 +39.10% / 最差 -11.38%。"
            ...
            _review_trigger = [
                ...
                "fund", "fund", "fund", "fund", "fund", "sse", "fund", "sse",
            ]
            _review_df = pd.DataFrame({
                "买入日": ["2019-02-25", "2019-05-06",
                          "2020-02-03", "2020-03-12", "2020-03-18",
                          "2020-07-16", "2020-08-17", "2022-04-25",
                          "2024-02-02", "2024-09-27", "2024-10-09",
                          "2024-10-15", "2024-10-22", "2024-11-01",
                ...
                "类型": ["混合型-灵活", "混合型-灵活", "指数型-股票",
                        "混合型-偏股", "混合型-偏股", "股票型", "混合型-灵活",
                        "指数型-股票", "指数型-股票", "指数型-股票",
                        "指数型-股票", "指数型-股票", "指数型-股票",
                        "指数型-股票", "混合型-偏股",
                        "混合型-偏股"],
                "波动率比值(近3月)": [1.33, 1.76, 2.71, 1.61, 1.63, 1.48, 1.18,
                                   1.84, 3.81, 1.47, 1.65, 1.27, 1.69, 1.83,
                                   4.88, 3.82],
                "恐慌阈值": [28.29, 29.37, 29.41, 29.41, 29.64, 30.93, 31.06,
                            29.71, 23.47, 23.11, 23.26, 23.39, 23.80, 24.54,
                            25.01, 24.88],
                "回撤控制线(%)": [7.53, 10.34, 15.94, 9.47, 9.66, 9.16, 7.33,
                                10.93, 17.89, 6.80, 7.68, 5.94, 8.04, 8.98,
                                24.41, 19.01],
                "大盘回撤线(%)": [5.66, 5.87, 5.88, 5.88, 5.93, 6.19, 6.21,
                                5.94, 4.69, 4.62, 4.65, 4.68, 4.76, 4.91,
                                5.00, 4.98],
                "冠军近3月涨幅(前日口径)": [
                    "+22.12%", "+51.68%", "+48.90%", "+51.96%", "+34.91%",
                    "+58.95%", "+62.52%", "+20.18%", "+17.10%", "+23.20%",
                    "+57.62%", "+58.24%", "+102.69%", "+98.98%", "+46.64%",
                    "+52.12%"],
                "卖出日": ["2019-04-29", "2019-08-05", "2020-03-09",
                          "2020-03-18", "2020-07-16", "2020-08-13",
                          "2020-09-09", "2022-06-22", "2024-06-05",
                          "2024-10-09", "2024-10-15", "2024-10-22",
                          "2024-11-01", "2024-11-22", "2025-12-16",
                          "2026-06-05"],
                "持有收益": ["+10.75%", "-1.36%", "+6.93%",
                            "-4.68%", "+32.49%",
                            "-0.72% (费后-1.22%)", "-6.06% (费后-6.56%)",
                            "+10.90%", "-3.21%", "+4.81% (费后+4.31%)",
                            "-9.88% (费后-11.38%)", "+23.72%",
                            "-0.68%", "+11.62%",
                            "+31.06%", "+39.10%"],
                "期间最高": ["+18.0%", "+6.8%", "+36.5%", "+0.7%", "+51.6%",
                            "+9.5%", "+2.2%", "+25.9%", "+18.2%", "+15.4%",
                            "+0.0%", "+33.5%", "+13.1%", "+20.6%", "+62.5%",
                            "+50.6%"],
                "期间最大回撤": ["6.3%", "7.7%", "21.7%", "5.7%", "12.6%",
                              "9.4%", "8.1%", "11.9%", "18.1%", "9.1%",
                              "9.9%", "7.3%", "12.2%", "7.5%", "21.4%",
                              "12.6%"],
                "同期上证": ["+3.4%", "-2.9%", "+7.2%", "-6.7%", "+17.6%",
                            "+3.4%", "-5.4%", "+11.6%", "+12.3%", "+5.5%",
                            "-1.8%", "+2.6%", "-0.4%", "-0.1%", "+23.5%",
                            "+5.6%"],
                "备注": [
                    ...
                    "(峰值+51.6%),基金线触发离场仍+32.49%——全表持有最久也"
                    "最赚的一笔",
                    "\"疯牛\"回调日(上证-4.5%)入场;峰值+9.5%即掉头,28天基金"
                    "线触发,费后-1.22%",
                    "非恐慌上涨日(上证+2.3%,亢奋型触发)追高榜首;23天基金线"
                    "触发,费后-6.56%——追高动量反转的典型亏损",
                    "上海封控恐慌底(上证-5.1%)入场;58天吃到煤炭超级周期一段"
                    "(峰值+25.9%),基金线触发+10.90%",
                    "微盘股崩盘尾声(上证-1.5%)入场;原始榜首为QDII(与A股弱"
                    "相关已剔除),改选北证50冠军(近3月+17.1%);124天基金线"
                    "(17.9%宽线扛住震荡)触发,-3.21%小亏",
                    "924行情亢奋周(上证+2.9%,非恐慌)追入保险指数;12天冠军"
                    "熄火,基金线触发,费后+4.31%",
                    "924行情后首个大跌日(上证-6.6%)追高近3月+57.6%的证券"
                    "冠军;6天基金线触发,费后-11.38%——全表最差,动量反转"
                    "最快",
                    "节后持续回调(上证-2.5%)入场北证50;7天基金线快速触发,"
                    "+23.72%——这轮动量延续最猛的一段,当天原地接回同一冠军"
                    "不收手续费",
                    "同一冠军(北证50)再度登顶,近3月涨幅已冲到+102.7%;10天"
                    "基金线触发,-0.68%——追第二棒动量到顶回落,当天原地接回"
                    "第三棒不收手续费",
                    "第三次连续接力同一冠军;21天后大盘线触发,+11.62%——三棒"
                    "连续接力的链条(10/15→11/22共38天)按累计持有天数只在"
                    "此收一次手续费,≥30天费率为0",
                    "\"对等关税\"暴跌日(上证-7.3%,近年单日最大跌幅之一)入场;"
                    "原始榜首同泰产业升级混合C 近3月涨幅含一笔无法解释的"
                    "净值跳变(已核实剔除),真实冠军为本基金(+46.64%);253天"
                    "基金线(24.41%宽线)一路扛住震荡未曾止损,直到大盘线触发,"
                    "+31.06%——期间原本该空仓的窗口被这笔长仓占满,后面一笔"
                    "科技主题冠军(永赢科技智选,原+30.14%)因此没有入场机会",
