...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # pct=0.90) + 冠军候选加相关系数过滤(min_corr=0.6, 与上证近3月
        # 日收益率相关系数够不上0.6的直接跳过, 不通过的往下一名找, 见
        # backtest_qvix.find_champion_on_date 的 sse_df/min_corr 说明)——
        # 用同一组数据实测过0.5/0.6/0.7三档, 0.6综合表现最好(见
        # backtest_qvix.py 里的"阈值组合回测参考快照")。此前(3年95分位、
        # 无相关系数过滤)的12笔复盘记录已被替换, 历史版本见 git 历史。
        #
        # 冠军排名复用 fetcher.compute_metrics_asof——与本页「基金列表」的
        # "截至日期"筛选完全同口径(按日收益率连乘计算区间收益,正确处理
        # 分红除权,自带单日|收益率|>30%异常值过滤),严格早于当日截断(T日
        # 决策只能看到T-1日收盘净值),已用 2020-07-16 001644 vs 009163
        # 交叉验证过。
        # 止损逐交易日检查,双线在买入日锁定——基金线=阈值/5×波动率比值,
        # 大盘线=阈值/5;波动率比值=基金日收益率std/大盘日收益率std(纯波动
        # 对比,不按相关系数加权,且剔除单日|收益率|>35%的净值重置/份额折算
        # 脏数据)。冠军候选池剔除:QDII/海外指数型(跟踪境外市场,与 QVIX/
        # 大盘恐慌-反弹逻辑脱钩)、有持有期锁定的基金(名称含"N年/N个月/N天
        # 持有(期)""滚动持有""定期开放""封闭式"等, 锁定期内实际赎不出来,
        # 回测里的卖出是假的)、"净值僵化-补涨"基金(排名窗口内连续≥5个
        # 交易日|日收益率|<0.05%紧接单日|收益率|>8%补涨/补跌, 判断为净值
        # 长期未按市值更新、事后集中补记, 而非真实动量)。连续接力同一只
        # 基金(上一腿卖出日=下一腿买入日且代码相同)视为未真实离场,中间腿
        # 不收手续费,只在链条最后一腿按"链条首次买入→该腿卖出"的累计持有
        # 天数收一次(而非按单腿天数)。数据 2018 年起(2015-2017 期权刚上市
        # 流动性薄、QVIX 计算噪声偏大, 已整段剔除, 见 fetcher.py)。
        with st.expander("📜 策略复盘:QVIX 2年90%信号 + 近3月冠军"
                         "(与大盘相关系数≥0.6) + 基金/大盘双止损逐日盯盘",
                          expanded=True):
            ...
            st.caption("11 笔已完成:胜率 54.5%(6/11),费后复利 +204.06%,"
                       "累计手续费 1.0%,平均持有 67 天,平均费后 +11.97%,"
                       "最佳 +39.10% / 最差 -9.40%。"
                       "橙色=触发基金回撤线离场,蓝色=触发大盘回撤线离场;"
                       "连续接力同一只基金视为未真实离场,中间腿不收手续费,"
                       "只在链条最后一腿按累计持有天数收一次。")
            _review_trigger = [
                "sse", "fund", "fund", "fund", "sse", "sse", "sse", "sse",
                "fund", "fund", "sse",
            ]
            _review_df = pd.DataFrame({
                "买入日": ["2020-07-06", "2020-07-16",
                          "2022-04-25", "2022-10-24", "2023-08-28",
                          "2024-02-05", "2024-09-26", "2024-10-09",
                          "2024-11-18", "2025-04-07", "2026-03-23"],
                "冠军(C类全市场,按前一交易日榜单)": [
                    "汇丰晋信智造先锋股票C (001644)",
                    "汇丰晋信智造先锋股票C (001644)",
                    ...
                    "万家经济新动能混合C (005312)",
                    "广发远见智选混合C (016874)"],
                "类型": ["股票型", "股票型", "指数型-股票", "混合型-灵活",
                        "指数型-股票", "混合型-偏股", "指数型-股票",
                        "指数型-股票", "指数型-股票", "混合型-偏股",
                        "混合型-偏股"],
                "波动率比值(近3月)": [2.66, 1.85, 1.92, 1.56, 2.05,
                                   1.04, 1.46, 3.11, 2.86, 2.89, 3.82],
                "恐慌阈值": [29.11, 29.60, 26.33, 23.24, 22.28,
                            22.28, 20.45, 20.51, 21.77, 21.42, 22.54],
                "回撤控制线(%)": [15.49, 10.95, 10.11, 7.25, 9.13,
                                4.63, 5.97, 12.75, 12.45, 12.38, 17.22],
                "大盘回撤线(%)": [5.82, 5.92, 5.27, 4.65, 4.46,
                                4.46, 4.09, 4.10, 4.35, 4.28, 4.51],
                "冠军近3月涨幅(前日口径)": [
                    "+46.75%", "+57.94%", "+14.12%", "+12.17%", "+10.51%",
                    "+4.93%", "+13.81%", "+63.61%", "+120.08%", "+34.44%",
                    "+57.21%"],
                "卖出日": ["2020-07-16", "2020-09-09",
                          "2022-06-20", "2022-12-19", "2023-10-19",
                          "2024-06-11", "2024-10-09", "2024-11-18",
                          "2024-12-16", "2025-11-21", "2026-06-05"],
                "持有收益": ["-0.20%", "-3.20%", "+14.38%", "-6.37%",
                            "-7.70%", "+22.49%", "+19.25% (费后+18.75%)",
                            "+36.76%", "-8.90% (费后-9.40%)", "+27.01%",
                            "+39.10%"],
                "期间最高": ["+11.2%", "+10.5%", "+27.6%", "+1.7%", "+0.0%",
                            "+23.9%", "+24.1%", "+46.8%", "+5.1%", "+49.0%",
                            "+50.6%"],
                "期间最大回撤": ["10.2%", "12.4%", "10.3%", "8.0%", "8.2%",
                              "2.6%", "3.9%", "11.6%", "13.3%", "14.8%",
                              "12.6%"],
                "同期上证": ["-3.7%", "+1.4%", "+13.2%", "+4.4%", "-3.0%",
                            "+12.1%", "+8.6%", "+2.0%", "+1.9%", "+23.8%",
                            "+5.6%"],
                "备注": [
                    "疫后修复行情中入场(相关系数筛选通过);10天后大盘线先"
                    "触发,-0.20%——当天原地接回同一冠军(下一条),视为未"
                    "真实离场不收手续费",
                    "接回同一冠军(第二次入场);55天后基金线触发,-3.20%"
                    "小亏,链条(07-06→09-09共65天)按最后一腿收费,超30天"
                    "免手续费",
                    "煤炭超级周期入场;与不设相关系数过滤时选中的另一只"
                    "煤炭ETF(国泰中证煤炭)是同题材不同基金,过滤后换成"
                    "相关系数更高的这只;56天基金线触发+14.38%",
                    "美联储加息尾声阶段入场;56天后基金线触发-6.37%——"
                    "相关系数达标不代表选中的这只基金本身跌得慢",
                    "相关系数0.82,是当期候选里最高的,涨幅也是最高,双重"
                    "达标但52天后大盘线触发-7.70%——说明那段时间大盘本身"
                    "在跌,相关系数高只代表跟跌得同步,不代表不跌",
                    "雪球产品集中平仓踩踏后入场;波动率比值仅1.04(全表"
                    "最低),127天扛住震荡直到大盘线触发+22.49%",
                    "924行情前夕入场;13天后大盘线触发+19.25%(费后"
                    "+18.75%)",
                    "924行情后首个大跌日追高近3月+63.61%的北证50冠军;"
                    "40天后大盘线触发+36.76%,全表次佳",
                    "同一北证50主题一个月后再度追高(涨幅已飙到"
                    "+120.08%,相关系数筛选未能规避主题见顶回落);28天后"
                    "基金线触发-8.90%(费后-9.40%),全表最差",
                    "\"对等关税\"暴跌日入场;228天基金线(12.38%宽线)一路"
                    "扛住震荡未曾止损,直到大盘线触发,+27.01%",
                    "上证单日-3.6%入场;74天大盘线触发离场,+39.10%——"
                    "全表最佳单笔收益",
                ],
            })
