...
with tab_sse:
    ...
    if sse_df is None or sse_df.empty:
        ...
        # 逻辑脱钩,选入会削弱大盘回撤线对该笔仓位的意义)。数据 2018 年起,
        ...
        with st.expander("📜 策略复盘:买入近3月冠军,基金/大盘双止损逐日盯盘"):
            ...
            st.caption("17 笔已完成:胜率 58.8%(10/17),费后复利 +251.62%,"
                       "累计手续费 6.0%,平均持有 50 天,平均费后 +8.58%,"
                       "最佳 +39.10% / 最差 -11.38%。"
            ...
            _review_df = pd.DataFrame({
                ...
                "持有收益": ["+10.75%", "-1.36%", "+6.93%",
                            ...
                            "-9.88% (费后-11.38%)", "+23.72% (费后+23.22%)",
                            "-0.68% (费后-1.18%)", "+11.62% (费后+11.12%)",
                            "+7.95%", "+30.14%", "+39.10%"],
                ...
                "备注": [
                    ...
                    "费后+23.22%——这轮动量延续最猛的一段",
                    "同一冠军(北证50)再度登顶,近3月涨幅已冲到+102.7%;10天"
                    "基金线触发,费后-1.18%——追第二棒动量到顶回落",
                    "第三次连续接力同一冠军;21天后大盘线触发,费后+11.12%"
                    "——连续三棒里最后守住了大盘线内的利润",
