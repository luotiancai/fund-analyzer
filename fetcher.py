...
def _fetch_index_cached(key: str, loader,
                        force_refresh: bool = False) -> Optional[pd.DataFrame]:
    """index_daily_cache 通用 cache-first 取数(12h TTL,与 fetch_sse_daily
    同契约):loader() 负责网络拉取并返回成品 DataFrame;失败退回旧缓存。"""
    conn = _conn()
    conn.execute("CREATE TABLE IF NOT EXISTS index_daily_cache ("
                 "key TEXT PRIMARY KEY, data TEXT, saved_at REAL)")
    row = conn.execute(
        "SELECT data, saved_at FROM index_daily_cache WHERE key=?", (key,)
    ).fetchone()
    ...
    try:
        ...
        logger.debug("index fetch failed (%s): %s", key, e)

    if df is not None and not df.empty:
        conn.execute(
            "INSERT OR REPLACE INTO index_daily_cache (key, data, saved_at) "
            "VALUES (?, ?, ?)",
            (key, df.to_json(orient="split", force_ascii=False), time.time()))
        conn.commit()
    elif row:
        df = _from_row(row)
    conn.close()
...
def _load_sina_index(symbol: str) -> pd.DataFrame:
    """sina 日线 → date/close/pct,与 fetch_sse_daily 同口径。"""
    ...
    df["pct"] = df["close"].pct_change() * 100.0
    return df[df["date"] >= INDEX_START].reset_index(drop=True)


def _load_optbbs_qvix(fn) -> pd.DataFrame:
    """optbbs QVIX 日线 → date/close。CSV 全日期占位,期权上市前为 NaN,
    dropna 后即真实数据起点(创业板 2022-09,科创板 2023-06)。"""
    raw = fn()
    return pd.DataFrame({
        "date": pd.to_datetime(raw["date"]).dt.strftime("%Y-%m-%d"),
        "close": pd.to_numeric(raw["close"], errors="coerce"),
...
def fetch_cyb_daily(force_refresh: bool = False) -> Optional[pd.DataFrame]:
    """创业板指(399006)daily,columns: date, close, pct。"""
    return _fetch_index_cached(
        "cyb", lambda: _load_sina_index("sz399006"), force_refresh)


def fetch_kcb_daily(force_refresh: bool = False) -> Optional[pd.DataFrame]:
    """科创50(000688)daily(2020-01 发布),columns: date, close, pct。"""
    return _fetch_index_cached(
        "kcb", lambda: _load_sina_index("sh000688"), force_refresh)


def fetch_cyb_qvix_daily(force_refresh: bool = False) -> Optional[pd.DataFrame]:
    """创业板ETF期权QVIX daily close,数据起点 2022-09(期权上市)。"""
    return _fetch_index_cached(
        "qvix_cyb", lambda: _load_optbbs_qvix(ak.index_option_cyb_qvix),
        force_refresh)


def fetch_kcb_qvix_daily(force_refresh: bool = False) -> Optional[pd.DataFrame]:
    """科创板(科创50ETF)期权QVIX daily close,数据起点 2023-06(期权上市)。"""
    return _fetch_index_cached(
        "qvix_kcb", lambda: _load_optbbs_qvix(ak.index_option_kcb_qvix),
        force_refresh)
