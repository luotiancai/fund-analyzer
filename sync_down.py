...
⚠️ **market 是混合权威, 同步前先想一下**: qvix/threshold 由线上每日跑批产出
(以云端为准), 但 **skew 列可能是你本地 backfill_skew.py 刚回填的**。同步是
整库文件替换, 直接拉会把本地还没推的 skew 整段冲掉 —— 2026-08-13 就这么丢过
一次约 790 天的回填, 只能重跑 20 分钟补回来。
正确顺序: **先 `python3 push_dbs.py market` 推上去, 再 sync_down**。
(每日跑批已经会顺带算当天 skew, 所以日常不再需要本地回填; 只有补历史缺口时
才会出现"本地比云端新"的情况。)
"""
